...
public class ModRepositoryTest : AbstractFilesystemTest
{
    ...
    public void ListMods_FindsDirectories()
    {
        ...
        Assert.Equivalent(
            new ModPackage[] {
                new("Dir1", "Dir1", Path.Combine(testDir.FullName, @"Enabled\Dir1"), true, 0),
                new("Dir2", "Dir2", Path.Combine(testDir.FullName, @"Enabled\Dir2"), true, 0)
            },
            modRepository.ListEnabledMods()
        );
        Assert.Equivalent(
            new ModPackage[] {
                new("Dir3", "Dir3", Path.Combine(testDir.FullName, @"Disabled\Dir3"), false, 0),
                new("Dir4", "Dir4", Path.Combine(testDir.FullName, @"Disabled\Dir4"), false, 0)
            },
